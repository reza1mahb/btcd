...
With Docker you can easily set up *btcd* to run your Bitcoin full node. You can find the official *btcd* Docker images on Docker Hub [binance-chain/btcd](https://hub.docker.com/r/binance-chain/btcd). The Docker source file of this image is located at [Dockerfile](https://github.com/binance-chain/btcd/blob/master/Dockerfile).
...
services:
  btcd:
    container_name: btcd
    hostname: btcd
    image: binance-chain/btcd:latest
    restart: unless-stopped
    volumes:
      - btcd-data:/root/.btcd
    ports:
